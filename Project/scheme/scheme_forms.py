...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 4
        # print(f'defining {signature} to be {scheme_eval(expressions.rest.first, env)}')
        ...
        # BEGIN PROBLEM 10
        name = signature.first         # 函数名 f
        formals = signature.rest       # 参数 (x)
        body = expressions.rest        # 函数体 (+ x 2)

        # 构造 (lambda (formals) body) 并调用 do_lambda_form
        lambda_expr = Pair(formals, body)
        lambda_proc = do_lambda_form(lambda_expr, env)
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        name = binding.first
        if name in s:
            raise SchemeError('duplicate name in let form: {0}'.format(name))
        s.add(name)
